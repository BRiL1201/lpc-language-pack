...
const PACKAGE_NAME = 'lpc-language-pack';
const CONFIG = {
...
class LPCHTML {
    ...
    this.config = {
      styles: {
        ...
        description:
          ...
          LPC HTML language constructs. This is made possible by taking advantage
  ...
  loadStylesFile() {
    ...
    this.stylesPath = [pkg.path, 'styles', 'LPCHTML.less'].join(path.sep);
    let styles;
    try {
  ...
  readDefaultColors() {
    let styles =
      atom.themes.lessCache.cssForFile(this.stylesPath, this.styles);
    for (let item in STYLES_REGEX) {
      item = item.toLowerCase();
      let regex = new RegExp(item + '[^:]+:\\s+([^;]+)');
      let match = styles.match(regex);
      if (!match) continue;
      atom.config.defaultSettings[PACKAGE_NAME].colors[item] = match[1];
    }
  }
  reloadStyles(colors) {
